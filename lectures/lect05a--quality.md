...
<!-- 
Proxies for requirements: test cases, documentation, user/client discussions, acceptance tests
-->
-   query: What if it meets the requirements, but customers are unhappy with it?
<!-- 
Need to capture customer wishes, preferably during project development.
Ways of doing this: Agile, Continuous dev, Min Viable Products etc
-->
-   query: What if it meets the requirements, but is insecure or in some other way illegal?
<!--
Ethical and legal responsibilities of professionals to produce high quality software (whether or not those reqs are explicitly captured)
-->
...
<!-- 
    Roughly speaking, a system is safe if it reasonably free of unacceptable hazards, and a system is secure if it is reasonably robust with respect to malicious threats. [Amman]

    Hazards are identified and characterised by criticality and risk [Pressman] (bad things that could happen, in the context of the whole system, eg car does not respond to brake pedal)

    Failure is a type of behaviour that we observe at run-time
    Externally observable incorrect behavior with respect to the requirements

    Faults (bugs, defects) A defect in a system’s static artifacts – usually source code – which at runtime will (if executed or encountered) cause the system to behave incorrectly.
-->
